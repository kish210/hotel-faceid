...
| داشبورد لحظه‌ای | فهرست مهمانان |
|---|---|
| ![داشبورد](docs/screenshots/dashboard.svg) | ![مهمانان](docs/screenshots/guests.svg) |

| مدیریت دوربین‌ها | هشدارها |
|---|---|
| ![دوربین‌ها](docs/screenshots/cameras.svg) | ![هشدارها](docs/screenshots/alerts.svg) |

| گزارش روزانه | نصب‌کننده ویندوز |
|---|---|
| ![گزارش‌ها](docs/screenshots/reports.svg) | ![نصب](docs/screenshots/installer.svg) |
...
### سرور پردازش (اجرای سامانه)

| مؤلفه | حداقل | توصیه‌شده |
|---|---|---|
| **سیستم‌عامل** | ویندوز ۱۰/۱۱ × 64 یا اوبونتو ۲۲.۰۴ LTS | ویندوز ۱۱ یا اوبونتو ۲۴.۰۴ LTS |
| **پردازنده (CPU)** | ۴ هسته (Core i5/Ryzen 5) | ۸ هسته (Core i7/Ryzen 7) |
| **حافظه RAM** | ۸ گیگابایت | ۱۶ گیگابایت |
| **پردازنده گرافیکی (GPU)** | — (اختیاری) | NVIDIA RTX 3060 یا بالاتر (CUDA 11+) |
| **فضای ذخیره‌سازی** | ۵۰ گیگابایت SSD | ۲۵۰ گیگابایت SSD NVMe |
| **شبکه** | ۱۰۰ Mbps | ۱ Gbps (سوئیچ PoE برای دوربین‌ها) |

> **بدون GPU:** سامانه روی CPU معمولی کار می‌کند. با ۴ دوربین و `FRAME_SAMPLE_RATE=5`،
> مصرف CPU حدود ۶۰–۸۰٪ روی Core i5 است. برای ۸+ دوربین همزمان، GPU توصیه می‌شود.

> **با GPU (NVIDIA CUDA):** پردازش ۴× سریع‌تر؛ `ONNX_PROVIDER=CUDAExecutionProvider` را
> در `.env` تنظیم کنید و `onnxruntime-gpu` را جایگزین `onnxruntime` کنید.

### دوربین‌ها

| مشخصه | الزامی | توضیح |
|---|---|---|
| **رزولوشن** | حداقل ۱۰۸۰p | برای تشخیص قابل اطمینان چهره |
| **پروتکل** | RTSP و/یا ONVIF Profile S | تقریباً همه دوربین‌های IP این را دارند |
| **ارتفاع نصب** | ۲ تا ۲.۵ متر | بالاتر = چهره از بالا دیده می‌شود و دقت کاهش می‌یابد |
| **زاویه** | حداکثر ۱۵ درجه کج | چهره باید تقریباً رو به دوربین باشد |
| **نور** | حداقل ۵۰ lux | در محیط کم‌نور دوربین IR یا محاسبات WDR ضروری است |
| **نرخ فریم** | ۱۵–۲۵ fps | کمتر از ۱۰ fps عبور سریع را از دست می‌دهد |

### مثال پیکربندی یک هتل ۵۰ اتاقه

```
سرور:        Core i7-12700  ·  16 GB RAM  ·  256 GB SSD  ·  GPU RTX 3060
دوربین‌ها:   ۲ عدد Hikvision DS-2CD2143G2 (لابی ورودی + خروجی)
             ۱ عدد Dahua IPC-HDW2831T (پارکینگ)
شبکه:        سوئیچ PoE 8-port  ·  VLAN مجزا برای دوربین‌ها
پایگاه داده: SQLite (پیش‌فرض) — تا ۱۰۰ هزار رویداد بدون مشکل
```
