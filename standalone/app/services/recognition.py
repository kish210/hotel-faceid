...
def find_match(db: Session, embedding: list[float]) -> tuple[Person | None, float | None]:
    ...
    """
    rows = db.execute(
        select(FaceEmbedding.embedding, FaceEmbedding.person_id)
    ...
    idx, similarity = vectors.nearest(embedding, [r[0] for r in rows])
    person = db.get(Person, rows[idx][1])
